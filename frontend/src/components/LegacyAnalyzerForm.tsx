...
import styles from './LegacyAnalyzerForm.module.css';

interface LegacyAnalyzerFormProps {
  files: File[];
  users: string;
  tasks: string;
  format: string;
...
export const LegacyAnalyzerForm: React.FC<LegacyAnalyzerFormProps> = ({
  files,
  users,
  tasks,
  format,
  ...
  const validate = useCallback((): boolean => {
    ...
    if (files.length === 0) newErrors.files = 'Please upload at least one screenshot or video';
    if (users.trim().length < MIN_CONTEXT_LENGTH) newErrors.users = `Please enter at least ${MIN_CONTEXT_LENGTH} characters`;
    if (tasks.trim().length < MIN_CONTEXT_LENGTH) newErrors.tasks = `Please enter at least ${MIN_CONTEXT_LENGTH} characters`;
    if (format.trim().length < MIN_CONTEXT_LENGTH) newErrors.format = `Please enter at least ${MIN_CONTEXT_LENGTH} characters`;
    setErrors(newErrors);
  ...
  const getButtonText = () => {
    ...
    if (files.length === 1) {
      const v = ['video/mp4', 'video/quicktime', 'video/webm'].includes(files[0].type);
      return v ? 'Analyze Video' : 'Analyze Design';
    }
  ...
  return (
    <form className={styles.form} onSubmit={handleSubmit}>
      ...
      >
        {isEstimating ? <span className={styles.spinner} /> : (
...
export default LegacyAnalyzerForm;

...
summary_headline / summary_narrative — THE EXECUTIVE BOTTOM LINE:
   ...
   `summary_narrative`: one SHORT paragraph — as long as needed to deliver the verdict and its essential caveats and NO longer (hard ceiling 5 sentences; typically 2–3; NEVER padded to a floor — a clear 2-sentence verdict beats a padded 4). The 5-sentence ceiling is ABSOLUTE and OUTRANKS the coverage list below: if covering every point would exceed it, drop the least-essential points until you are within it — but NEVER drop the caveats (point 4). Do NOT write one sentence per point; merge aggressively — this is a bottom line, not a write-up. The length rule is the single source of truth and holds identically in BOTH Brief and Standard modes. Cover, compactly, in this order (2 and 3 only where they materially change the picture):
     1) The bottom line, plainly — does the design support the stated users and tasks, and how well (what it gets right, where it breaks down). Always include.
     2) What most gets in the users' way — described as a person would experience it, grouped, not an exhaustive list. Include only where it materially adds beyond the bottom line.
     3) What is worth considering to improve the outcome — advisory and results-oriented ("the biggest lever is…", "consider…"), never a rigid instruction. Include only where there is a clear, material lever.
     4) The one or two caveats a decision-maker must hear — usually what could NOT be judged from a static screenshot or from the context given, and would change the picture if known. ALWAYS include when present — this is the one point never dropped for length.
